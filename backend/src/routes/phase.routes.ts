...
export async function phaseRoutes(app: FastifyInstance) {
  ...
  app.get('/api/phases', async (_request: FastifyRequest, reply: FastifyReply) => {
    try {
  ...
  app.get('/api/phases/:id', async (request: FastifyRequest<{ Params: IdParams }>, reply: FastifyReply) => {
    try {
  ...
  app.post('/api/phases', async (request: FastifyRequest<{ Body: CreatePhaseBody }>, reply: FastifyReply) => {
    try {
      const data = request.body;
      
      if (!data.name || !data.description || !data.monthRange || !data.duration) {
        return reply.status(400).send({ error: 'Nome, descrição, período e duração são obrigatórios' });
      }
  ...
  app.put('/api/phases/:id', async (request: FastifyRequest<{ Params: IdParams; Body: UpdatePhaseBody }>, reply: FastifyReply) => {
    try {
      const { id } = request.params;
      const data = request.body;
  ...
  app.delete('/api/phases/:id', async (request: FastifyRequest<{ Params: IdParams }>, reply: FastifyReply) => {
    try {
      const { id } = request.params;
